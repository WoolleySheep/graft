...
def test_update_concrete_task_progress_failure_started_dependent_tasks(
    ...
    expected_subsystem.add_task(dependee_task)
    expected_subsystem.add_task(dependent_task)
    expected_subsystem.add_dependency(dependee_task, dependent_task)
    expected_subsystem.set_progress(dependee_task, tasks.Progress.COMPLETED)
    expected_subsystem.set_progress(dependent_task, started_progress)

    expected_started_downstream_task_to_progress_map = {
        dependent_task: started_progress
    }

    data_layer_mock.load_system.return_value = system
    logic_layer = logic.StandardLogicLayer(data_layer=data_layer_mock)

    with pytest.raises(tasks.DownstreamTasksHaveStartedError) as e:
        logic_layer.update_concrete_task_progress(
            task=dependee_task, progress=incomplete_progress
        )
    assert e.value.task == dependee_task
    assert (
        e.value.started_downstream_tasks_to_progress_map
        == expected_started_downstream_task_to_progress_map
    )
    assert e.value.subsystem == expected_subsystem
...
def test_update_concrete_task_progress_failure_started_dependent_tasks_of_superior_tasks(
    ...
    expected_subsystem.add_hierarchy(task0, task1)
    expected_subsystem.add_dependency(task0, task2)
    expected_subsystem.set_progress(task1, tasks.Progress.COMPLETED)
    expected_subsystem.set_progress(task2, started_progress)

    expected_started_downstream_task_to_progress_map = {task2: started_progress}
...
def test_update_concrete_task_progress_failure_incomplete_dependee_tasks(
    ...
    expected_subsystem.add_task(dependent_task)
    expected_subsystem.add_dependency(dependee_task, dependent_task)
    expected_subsystem.set_progress(dependee_task, incomplete_progress)

    expected_incomplete_upstream_tasks_to_progress_map = {
    ...
    with pytest.raises(tasks.UpstreamTasksAreIncompleteError) as e:
        logic_layer.update_concrete_task_progress(
            task=dependent_task, progress=started_progress
        )
    assert e.value.task == dependent_task
    assert (
        e.value.incomplete_upstream_tasks_to_progress_map
        == expected_incomplete_upstream_tasks_to_progress_map
    )
    assert e.value.subsystem == expected_subsystem
...
def test_update_concrete_task_progress_failure_incomplete_dependee_tasks_of_superior_tasks(
    ...
    expected_subsystem.add_task(task2)
    expected_subsystem.add_hierarchy(task0, task1)
    expected_subsystem.add_dependency(task2, task0)
    expected_subsystem.set_progress(task2, incomplete_progress)

    expected_incomplete_upstream_tasks_to_progress_map = {task2: incomplete_progress}
